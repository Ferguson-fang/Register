...
CDesTest::CDesTest(CString mingwen,CString miyao)
{
	MINGWEN=mingwen;
	MIYAO = miyao;
}
 
CDesTest::CDesTest(CString m_key)
{
	MIYAO = m_key;
}


CDesTest::~CDesTest(void)
{
}
...
void CDesTest:: jiqimaerjinzhi(CString m_str)
{
	...
	for(i=0;i<16;i++)
	{
		...
		for(j=3;j>=0;j--)		//获取一个字符的二进制码，从高位到低位放入数组
		{
			str[j]=ch%2;
			ch=ch/2;
		}
...
void CDesTest::Mingwenerjinzhi()
{
	...
	{
		ch=MINGWEN.GetAt(i);
		for(j=7;j>=0;j--)		//获取一个字符的二进制码，从高位到低位放入数组
		{
			str[j]=ch%2;
			ch=ch/2;
		}
...
void CDesTest::initial()//生成明文的IP和密钥的16个子密钥
{
	//FILE *fp;
...
void CDesTest::DESDIEDAI()
{
	...
	for(i=1;i<=64;i++)
		t1[i]=t[IP_1[i-1]];		//tl里面存储着密文的二进制数
	MEWEN="";
	//for(i=1;i<=64;i++)
	//{
	//	MEWEN=MEWEN+(char)(48+t1[i]); //转换为字符
	//	if(i%8==0)
	//		MEWEN=MEWEN+" ";
	//}
	int ch = 0;
	...
	{
		ch = 0;
		ch = t1[i]*8+t1[i+1]*4+t1[i+2]*2+t1[i+3];
		if (ch >= 0 && ch <= 9)
			ch += 48;
...
void CDesTest::DESJIEMI()
{
	...
	for(i=0;i<=7;i++)
	{
		for(j=8;j>=1;j--)
		{
			ch=ch+t1[i*8+j]*(int)pow((float)2,8-j);
		}
		YUANWEN=YUANWEN+(char)ch;
		ch=0;
	}
	//UpdateData(false);
}
...
void CDesTest::initialJIEMI()//生成明文的IP和密钥的16个子密钥
{
	//FILE *fp;
